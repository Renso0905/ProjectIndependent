...
export default function LoginRBT() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const { redirect } = await api.login(username, password, "RBT");
      window.location.href = redirect || "/dashboard/rbt";
    } catch (err: any) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-6">
      <form className="border rounded-2xl p-6 w-full max-w-sm space-y-3" onSubmit={onSubmit}>
        <h1 className="text-2xl font-semibold text-center">RBT Login</h1>
